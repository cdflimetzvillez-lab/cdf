...
export default function TableGagnants({ lignes }: { lignes: ParticipationRoue[] }) {
  ...
  function exporter() {
    const rows = [['date', 'code', 'lot', 'prenom', 'nom', 'email', 'telephone', 'retire'],
      ...lignes.map((l) => [fmt(l.created_at), l.code ?? '', l.roue_lots?.nom ?? '', l.prenom ?? '', l.nom ?? '', l.email ?? '', l.telephone ?? '', l.retire_le ? 'oui' : 'non'])];
