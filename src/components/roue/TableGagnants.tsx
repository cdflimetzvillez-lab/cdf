...
export default function TableGagnants({ lignes }: { lignes: ParticipationRoue[] }) {
  ...
  return (
    <div className="panel" style={{ opacity: pending ? .7 : 1 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '.8rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
      ...
      <table className="tbl cartes">
        ...
        <tbody>
          {lignes.map((l) => (
            <tr key={l.id}>
              <td data-l="Date">{fmt(l.created_at)}</td>
              <td data-l="Code" className="mono">{l.code}</td>
              <td data-l="Lot">{l.roue_lots?.nom ?? '—'}</td>
              <td data-l="Gagnant">{l.prenom ? `${l.prenom} ${l.nom ?? ''}` : <span style={{ color: '#6b6560' }}>non renseigné</span>}</td>
              <td data-l="Contact" style={{ fontSize: '.8rem', wordBreak: 'break-all' }}>{l.email ? <>{l.email}<br /></> : null}{l.telephone}</td>
              <td data-l="État">{l.annulee_le ? <span className="pill off" title={l.motif_annulation ?? ''}>annulé · {l.motif_annulation?.startsWith('Non réclamé') ? 'non réclamé' : 'doublon'}</span> : (l.reclame_le ? <span className="pill on">valide</span> : <span className="pill new">en attente de coordonnées</span>)}</td>
              <td data-l="Retiré">{l.annulee_le ? '—' : <button className={`pill ${l.retire_le ? 'done' : 'off'}`} style={{ cursor: 'pointer' }} onClick={() => start(() => marquerRetire(l.id, !l.retire_le))}>{l.retire_le ? 'oui' : 'non'}</button>}</td>
            </tr>
          ))}
